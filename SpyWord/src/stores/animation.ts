...
export const useAnimationStore = defineStore('animation', () => {
  ...
  function addAnimation(animation: Animation) {
    ...
    animationList.value.push({
      duration:
        timerAnimation.value[animation.name]! === undefined
          ? timerAnimation.value.default!
          : timerAnimation.value[animation.name]!,
      name: animation.name,
  ...
  const hookAnimation = (animation: Animation) => {
    const gameStore = useGameStore()
    switch (animation.name) {
      case 'newRound':
        gameStore.resetRoles()
        break
    }
  }
